// app/data-warehouse/analytics/amr/page.tsx
"use client";
...
export default function AMRAnalyticsPage() {
    ...
    (async () => {
      try {
        ...
        for (const ind of indicators) {
          ...
          const filtered = outs.filter((o) => !!o.isVisibleOnAMR);
          if (filtered.length) acc.push({ indicator: ind.name, outputs: filtered });
        }
        setGroups(acc);
      } catch (e: any) {
        setError(e?.message || "Failed to load AMR analytics");
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">AMR Analytics</h1>
        <p className="text-sm text-gray-600">
          Outputs that were flagged to show on the <strong>AMR</strong> analytics view.
        </p>
      </div>

      {loading ? (
        <div className="flex items-center justify-center text-gray-600 py-16">
          <FiRefreshCw className="animate-spin mr-2" /> Loading AMR outputs…
        </div>
      ) : error ? (
        <div className="text-center text-red-600 py-10">{error}</div>
      ) : groups.length === 0 ? (
        <div className="text-center text-gray-600 py-10">
          No outputs are currently visible for AMR.
        </div>
      ) : (
            ...
            <div key={g.indicator} className="bg-white rounded-xl shadow border border-gray-200">
              <div className="px-5 py-4 border-b border-gray-100">
                ...
                <div className="text-xs text-gray-500">
                  Showing outputs flagged for AMR.
                </div>
              </div>
