      ...
      <div className="flex flex-col w-64 border-r border-gray-200 bg-white">
        <div className="flex flex-col h-full overflow-y-auto">
          ...
          <div className="flex items-center flex-shrink-0 px-4 py-5">
            <h1 className="text-xl font-bold text-gray-900">Data Capturing</h1>
          </div>
                      ...
                      <>
                        ...
                        <button
                          ...
                          className={`group flex items-center w-full px-2 py-2 text-sm font-medium rounded-md transition-colors ${
                            openMenus[item._id] ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                          }`}
                          onClick={() => toggleMenu(item._id)}
                        >
                          <span className={`mr-3 flex-shrink-0 ${openMenus[item._id] ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-500'}`}>
                            <IconPreview name={item.icon} />
                          </span>
                          {item.name}
                          <span className="ml-auto">
                            <FiChevronDown
                              className={`h-4 w-4 transition-transform ${openMenus[item._id] ? 'rotate-180' : ''}`}
                            />
                          </span>
                        </button>
                              ...
                              .map((sub) => (
                                <Link
                                  ...
                                  className={`group flex items-center px-2 py-2 text-sm font-medium rounded-md transition-colors ${
                                    isActive(sub.href) ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                                  }`}
                                >
                                  <span
                                    className={`mr-3 flex-shrink-0 ${
                                      isActive(sub.href) ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-500'
                                    }`}
                                  >
                                    <IconPreview name={sub.icon} className="h-4 w-4" />
                                  </span>
                                  {sub.name}
                                </Link>
                              ))}
