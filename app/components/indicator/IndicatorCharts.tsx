...
export default function IndicatorCharts({ className = '', onEditIndicator, onViewOutput, onDeleteIndicator }: IndicatorChartsProps) {
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="bg-white p-6 rounded-xl shadow-md">
        <h3 className="text-lg font-semibold mb-4 text-gray-800">Recent Indicators</h3>
                ...
                <tr key={indicator.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                      {indicator.nodeName || 'No Node'}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {indicator.selectedVariables?.length || 0}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                      indicator.lastStatus === 'success' 
                        ? 'bg-green-100 text-green-800'
                        : indicator.lastStatus === 'failed'
                        ? 'bg-red-100 text-red-800'
                        : 'bg-yellow-100 text-yellow-800'
                    }`}>
                      {indicator.lastStatus || 'pending'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {indicator.executionCount || 0}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {indicator.lastExecuted 
                      ? new Date(indicator.lastExecuted).toLocaleDateString()
                      : indicator.dataCounts?.lastAnalyzed 
                        ? new Date(indicator.dataCounts.lastAnalyzed).toLocaleDateString()
                        : 'Never'
                    }
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center gap-2">
                      {onViewOutput && (
                        <button
                          onClick={() => onViewOutput(indicator)}
                      ...
                      <button
                        ...
                        disabled={deletingIndicator === indicator.id}
                        className="text-red-600 hover:text-red-900 p-1 rounded disabled:opacity-50"
                        title="Delete indicator"
                      >
                        {deletingIndicator === indicator.id ? (
