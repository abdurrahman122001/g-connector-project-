...
import { getApiPoints, ApiPoint } from "../services/dataWarehouseApi";
...
function CreateApiPointModal({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (normalizedVars.length === 0) {
      setError(
        "Variables are invalid. Add at least one variable name (or paste a list/JSON)."
      );
      return;
    }
    ...
    const payload = {
      ...
      nodeId: nodeId, // ← inject the selected node's ID
    };
  ...
  return (
    <div
      className="fixed inset-0 z-50 bg-black bg-opacity-30 overflow-y-auto"
      aria-modal="true"
      role="dialog"
    >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Public path will be{" "}
                    <code>/api/{form.slug || "your-slug"}</code>
                  </p>
                </div>
                <div>
                  ...
                  {form.variables.map((v, idx) => (
                    <div key={idx} className="flex gap-2 mb-2">
                      <input
                        ...
                        value={v.name}
                        onChange={(e) =>
                          handleVarChange(idx, "name", e.target.value)
                        }
                        required
                      />
                      <input
                        ...
                        value={v.type}
                        onChange={(e) =>
                          handleVarChange(idx, "type", e.target.value)
                        }
                      />
                      <input
                        type="text"
                        ...
                        value={v.description}
                        onChange={(e) =>
                          handleVarChange(idx, "description", e.target.value)
                        }
                      />
                      <label className="flex items-center gap-1">
                        <input
                          ...
                          checked={v.required}
                          onChange={(e) =>
                            handleVarChange(idx, "required", e.target.checked)
                          }
                        />
                        Required
                      </label>
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {loadedFieldCount !== null && loadedFieldCount === 0 && (
                    <span className="text-xs text-gray-500">
                      No variables were detected for this node. You can add them
                      manually.
                    </span>
                  )}
                </div>
...
export default function DataWarehousePage() {
  ...
  const [selectedNodeForApi, setSelectedNodeForApi] = useState<any | null>(
    null
  );
    ...
    try {
      ...
      if (createdIndicatorId) {
        try {
          ...
          console.warn(
            "Script execution failed, but indicator was created:",
            executeError
          );
        }
      }
