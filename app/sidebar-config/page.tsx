...
export default function SidebarConfigPage() {
  ...
  const startEditSub = (parentId: string, s: SubItem) => {
    setEditingSub((prev) => ({ ...prev, [s._id]: { ...s, roles: [...(s.roles || [])] } }));
  };
        ...
        <div className="grid gap-3 grid-cols-1 md:grid-cols-5 items-center">
          ...
          <div className="w-full">
            <Select
              ...
              value={ROLE_OPTIONS.filter((r) => parentForm.roles.includes(r.value))}
              onChange={(vals) =>
                setParentForm((f) => ({
                  ...f,
                            ...
                            <div className="flex-1 grid gap-2 grid-cols-1 md:grid-cols-4">
                              ...
                              <Select
                                ...
                                value={ROLE_OPTIONS.filter((r) =>
                                  (editingParent[item._id]?.roles as string[])?.includes(r.value),
                                )}
                                onChange={(vals) =>
                                  setEditingParent((prev) => ({
                                    ...prev,
                                    [item._id]: { ...prev[item._id], roles: vals.map((v) => v.value) },
                                  }))
                                }
                              />
                            </div>
                          ...
                          {(subProvided) => (
                            <div ref={subProvided.innerRef} {...subProvided.droppableProps}>
                              {item.subItems
                                ...
                                .sort((a, b) => a.order - b.order)
                                .map((s, sidx) => (
                                  <Draggable draggableId={s._id} index={sidx} key={s._id}>
                                    {(sdrag) => (
                                      <div
                                        ref={sdrag.innerRef}
                                      ...
                                      >
                                        {editingSub[s._id] ? (
                                          <div className="flex-1 grid gap-2 grid-cols-1 md:grid-cols-4">
                                            <TextField
                                              placeholder="Name"
                                              value={editingSub[s._id]?.name ?? ''}
                                              onChange={(e) =>
                                                setEditingSub((prev) => ({
                                                  ...prev,
                                                  [s._id]: { ...prev[s._id], name: e.target.value },
                                                }))
                                              }
                                            />
                                            <TextField
                                              placeholder="Href"
                                              value={editingSub[s._id]?.href ?? ''}
                                              onChange={(e) =>
                                                setEditingSub((prev) => ({
                                                  ...prev,
                                                  [s._id]: { ...prev[s._id], href: e.target.value },
                                                }))
                                              }
                                            />
                                            <div className="flex items-center gap-2">
                                              <IconPreview name={editingSub[s._id]?.icon as string} />
                                              <TextField
                                                placeholder="Icon"
                                                value={editingSub[s._id]?.icon ?? ''}
                                                onChange={(e) =>
                                                  setEditingSub((prev) => ({
                                                    ...prev,
                                                    [s._id]: { ...prev[s._id], icon: e.target.value },
                                                  }))
                                                }
                                              />
                                            </div>
                                            <Select
                                              ...
                                              options={ROLE_OPTIONS}
                                              value={ROLE_OPTIONS.filter((r) =>
                                                (editingSub[s._id]?.roles as string[])?.includes(r.value),
                                              )}
                                              onChange={(vals) =>
                                                setEditingSub((prev) => ({
                                                  ...prev,
                                                  [s._id]: { ...prev[s._id], roles: vals.map((v) => v.value) },
                                                }))
                                              }
                                            />
                                          </div>
                                          ...
                                          <div className="flex items-center gap-3">
                                            <IconPreview name={s.icon} className="w-4 h-4 text-gray-400" />
                                            <div>
                                              <div className="text-sm">{s.name}</div>
                                              <div className="text-xs text-gray-500">{s.href}</div>
                                              {s.roles?.length > 0 && (
                                        ...
                                        <div className="flex items-center gap-2 ml-3">
                                          {editingSub[s._id] ? (
                                            <>
                                              <SmallBtn
                                                variant="primary"
                                                onClick={() => saveEditSub(item._id, s._id)}
                                                title="Save"
                                              >
                                                <FiSave />
                                              </SmallBtn>
                                              <SmallBtn onClick={() => cancelEditSub(s._id)} title="Cancel">
                                                <FiX />
                                              </SmallBtn>
                                            </>
                                          ) : (
                                            <>
                                              <SmallBtn onClick={() => startEditSub(item._id, s)} title="Edit">
                                                <FiEdit2 />
                                              </SmallBtn>
                                              <SmallBtn
                                                variant="danger"
                                                onClick={() => deleteSub(item._id, s._id)}
                                                title="Delete"
                                              >
                                                <FiTrash2 />
                                              </SmallBtn>
                              ...
                              <div className="ml-8 border-t bg-gray-50 p-2">
                                <div className="grid gap-2 grid-cols-1 md:grid-cols-4 items-center">
                                  <TextField
                                    ...
                                    onChange={(e) =>
                                      setSubForms((prev) => ({
                                        ...prev,
                                        [item._id]: {
                                          ...(prev[item._id] || { href: '', icon: 'FiCircle', roles: [] }),
                                          name: e.target.value,
                                        },
                                      }))
                                    }
                                  ...
                                  <TextField
                                    ...
                                    onChange={(e) =>
                                      setSubForms((prev) => ({
                                        ...prev,
                                        [item._id]: {
                                          ...(prev[item._id] || { name: '', icon: 'FiCircle', roles: [] }),
                                          href: e.target.value,
                                        },
                                      }))
                                    }
                                  ...
                                  <Select
                                    isMulti
                                    options={ROLE_OPTIONS}
                                    value={ROLE_OPTIONS.filter((r) => subForms[item._id]?.roles?.includes(r.value))}
                                    onChange={(vals) =>
                                      setSubForms((prev) => ({
                                        ...prev,
                                        [item._id]: {
                                          ...(prev[item._id] || { name: '', href: '', icon: 'FiCircle' }),
                                          roles: vals.map((v) => v.value),
                                        },
                                      }))
                                    }
